...
public class LikeMark extends CollaborativeMark{
	
	private MyPopup popupMarkers = null;
	
	public LikeMark(){
		super();
		icon = new Icon(IconType.THUMBS_UP);
		markers = new HTML();
	...
	public void processClick(){
		...
		if(canClick){
			if(isClicked == EMarkingConfiguration.EMARKING_COLLABORATIVE_BUTTON_CLICKED){
				// remove like
				String sendVars = "type=1&status=0&markerid="+EMarkingConfiguration.getMarkerId()+"&commentid="+mark.getId();
				...
				// add like
				String sendVars = "type=1&status=1&markerid="+EMarkingConfiguration.getMarkerId()+"&commentid="+mark.getId();
				AjaxRequest.ajaxRequest("action=clickcollaborativebuttons&"+sendVars, new AsyncCallback<AjaxData>() {
					...
					public void onSuccess(AjaxData result) {
						...
						if(buttons.getWidget(1) instanceof DisLikeMark && 
								((DisLikeMark) buttons.getWidget(1)).getIsClicked() == EMarkingConfiguration.EMARKING_COLLABORATIVE_BUTTON_CLICKED){
							((DisLikeMark) buttons.getWidget(1)).processClick();
						}
						setIsClicked(1);
						addValue(1);
						setStyleClicked();
